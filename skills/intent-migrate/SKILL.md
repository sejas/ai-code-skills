...
7. Perform the migration:
   - Create `{CLAUDE_INTENTS_FOLDER}/in-progress/` and `{CLAUDE_INTENTS_FOLDER}/done/` if they don't exist
   - Move each intent folder from `.sejas/in-progress/` to `{CLAUDE_INTENTS_FOLDER}/in-progress/` with new name
